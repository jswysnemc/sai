...
function M.input_prompt(issue, target)
    return string.format(
        "用户输入法问题：\n%s\n\n目标软件：%s\n\n请按照系统提示词中的流程完成诊断。优先调用 lua__diagnostic-evidence__check_issue 收集输入法证据；如果目标软件框架或特殊行为不清楚，可以使用 lua__fcitx-wiki__fcitx5_input_method_wiki_qurey、知识库和网络搜索。最终只输出诊断报告。",
