...
local function preview(enabled, path)
    ...
    for _, tool in ipairs(tools) do
        if tool.name == "lua__image-display__print_image" then
            local printed, err = pcall(sai.tools.call, "lua__image-display__print_image", {image=path}, {timeout_ms=20000})
...
function M.save(config, prompt, image)
    ...
    return {
        ...
        assistant_instruction=printed
            and "The generated image has already been printed in the terminal. In your final response, include the exact local image path from final_response_must_include_path. Do not call lua__image-display__print_image again unless the user asks to print it again."
            or "The generated image was saved to disk. In your final response, include the exact local image path from final_response_must_include_path. Do not call lua__image-display__print_image unless the user explicitly asked to display, print, render, preview, or show it.",
    }
end
