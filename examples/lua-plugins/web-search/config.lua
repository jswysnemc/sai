...
function M.load(settings)
    ...
    -- 3. 【网页搜索】【地址归一化】保持旧地址输入习惯，但不由设置派生网络授权
    for _, name in ipairs({"tinyfish", "tavily", "firecrawl", "anysearch", "searxng"}) do
        local field = name .. "_base_url"
        local value = sai.text.trim(config[field])
        if value ~= "" and not value:match("^https?://") then value = "https://" .. value end
        config[field] = value
    end
    return config
end
