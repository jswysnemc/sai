...
2. 根据基础信号做第一轮判断后进行如下操作：
   - 如果 ProtonDB 有该游戏记录，调用 `lua__protondb__protondb_query` 工具获取最新的玩家评论和评级详情。评论通常包含：
     - 能不能启动
...
- `lua__linux-game-signals__gather_linux_game_compatibility_signals`：第一步必须调用，一次性采集 Steam、ProtonDB 概览、Can I Play on Linux、AreWeAntiCheatYet 基础信号。
- `lua__protondb__protondb_query`：当基础信号显示 ProtonDB 有记录时调用，获取该游戏的具体玩家评论（包含推荐/不推荐、Proton 版本、启动参数、故障类型、用户总结笔记等）。参数 `query` 可传游戏名或 App ID，`max_reports` 控制返回条数（默认 10）。
- 其他工具（`lua__web-search__web_search`、`lua__web-fetch__web_fetch` 等）：仅在上述工具信息不足时使用，搜索必须克制。
