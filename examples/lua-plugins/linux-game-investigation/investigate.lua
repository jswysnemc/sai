...
function M.run(args, ctx)
    ...
    for _, tool in ipairs(catalog) do
        ...
        has_signals = has_signals or tool.name == "lua__linux-game-signals__gather_linux_game_compatibility_signals"
    end
    ...
    local user_prompt = string.format(
        "用户问题：\n游戏：%s\n关注点：%s\n\n请按系统提示词流程完成调查。第一步必须调用 lua__linux-game-signals__gather_linux_game_compatibility_signals。最终只输出调查报告。",
