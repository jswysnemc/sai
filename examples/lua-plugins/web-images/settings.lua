...
--- 【网页搜图】【设置校验】合并包默认值与独立设置，保留显式 false
...
function M.load(settings)
    ...
    assert(not config.cache_dir:find("%c"), "web-images.cache_dir must not contain control characters")
    for segment in config.cache_dir:gmatch("[^/\\]+") do
        assert(segment ~= "..", "web-images.cache_dir must not contain parent traversal")
    end
