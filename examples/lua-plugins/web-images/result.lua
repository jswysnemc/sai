...
function M.finish(config, ctx, query, stored, rejected, vision, preview, preview_count)
    ...
    if preview and preview_count > 0 then
        for _, tool in ipairs(sai.tools.list()) do
            if tool.name == "lua__image-display__print_image" then visible = true; break end
        end
    end
    ...
    if visible then
        ...
        for index = 1, math.min(#stored, preview_count) do
            local ok, error = pcall(sai.tools.call, "lua__image-display__print_image", {image=stored[index].local_path})
    ...
    return {success=#images > 0, query=query, count=#images, result_role="downloaded_image_candidates",
        ...
        assistant_instruction=visible
            and "The searched images have been downloaded and previewed in the terminal when possible. In your final response, include the local_path values for reusable images. Do not call lua__image-display__print_image again for already printed images unless the user asks."
            or "The searched images have been downloaded to local_path. In your final response, include useful local_path and page_url values. Call lua__image-display__print_image only if the user explicitly asks to render or preview them.",
    }
end
