...
local function execute(config, name, args, ctx)
    ...
    if name == "add" or name == "add-text" then
        local files
        if name == "add" then
            files = require("mutations.source").add(config, values.required(args, "path"), args.name)
        else
            files = transaction.with(config, true, function()
                return sai.json.array({require("mutations.import").content(config,
                    values.required(args, "name"), assert(args.content, "content is required"))})
            end)
        end
...
--- @param config table 配置
--- @return nil 十个用户命令完成注册
function M.register(config)
    for _, name in ipairs({"add", "add-text", "list", "search", "find", "read", "remove", "reindex", "stats", "embed-reindex"}) do
