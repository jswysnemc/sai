...
--- 【知识库配置】【实例快照】只解析本包设置，不继承应用供应商或业务配置
...
function M.resolve(input)
    ...
    for key, value in pairs(input) do
        assert(config[key] ~= nil, "unknown knowledge-base setting: " .. tostring(key))
        config[key] = value
    end
    ...
    assert(config.data_dir ~= "" and #config.data_dir <= 4096 and not config.data_dir:find("%c"), "invalid knowledge-base.data_dir")
    for segment in config.data_dir:gmatch("[^/\\]+") do
        assert(segment ~= "..", "knowledge-base.data_dir must not contain parent traversal")
    end
    ...
    config.provider = require("embedding.provider").validate(config.provider, config.embedding_provider_id)
    return config
end
