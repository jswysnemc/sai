...
`list()` 等价于 `list({offset=0, limit=16})`。`offset` 为 0–256 的整数，`limit` 为 1–16 的整数，未知选项报错。结果按创建时间、任务 ID 升序排列；并发新增和历史清理可能改变后续分页位置，各页不构成同一事务快照。较大的 offset 用于容纳闹钟过渡期的旧记录，不扩大新建任务配额。

## 任务记录与状态

新任务 ID 为 `job-` 加 32 位小写十六进制 UUID；旧闹钟使用同形摘要标识。其他插件无法通过这个 ID 查询或取消记录。
...
原生调度状态是最后一次持久提交的结果，不是实时进程存活证明。进程异常退出后，记录可能仍显示 `scheduled`、`running` 或 `cancelling`，查询不会自动修正或重新执行。旧闹钟兼容查询另做进程身份观察，详见下文。
...
以上配额只统计 `plugin-jobs` 中的新记录。`alarm` 另可查询最多 128 条旧记录，总列表最多 256 条；旧任务不参与新建任务的活动额度或历史驱逐。
...
原生调度取消只修改本插件任务状态，不根据持久 PID 发送终止信号。未开始的任务直接记录为 `cancelled`；执行中的任务先进入 `cancelling`，工作进程观察状态后释放命令 Future，再记录终态。执行锁已经释放时，可以直接确认取消。旧闹钟使用下文所述的稳定句柄兼容取消。
...
创建时绑定插件 ID、可信工作目录、宿主路径、当前语言和完整版本摘要。摘要覆盖清单、Lua 源码、设置与授权；内部记录只保存摘要、路径、语言、任务参数和有界结果，不复制插件设置或凭据。公开任务不包含宿主路径、语言和摘要。

新记录的语言只能是 `en-US` 或 `zh-CN`，工作进程启动参数、显式恢复和到期校验都复用该值。宿主在同步作用域中按创建语言重建派生设置，不修改进程全局语言或其他线程；实际包设置仍参与完整摘要。这样可以保留 CLI `--lang` 覆盖，同时避免环境语言不同导致无实际配置变化的任务失败。旧记录未保存语言时保留原环境检测行为。
...
三个管理操作都输出 JSON，分别为 `{"jobs":[...]}`、`{"id":"...","cancelled":true}` 和 `{"task":{...}}`。管理列表可以读取全部 128 条新记录，以及 `alarm` 最多 128 条旧记录。列表和取消不要求插件仍然启用或安装，恢复待执行任务必须重新通过授权与版本检查。管理操作无需模型服务。
