...
| 探测本构建已实现的宿主功能 | `sai.host.info`、`sai.host.has` | 初始化及回调均可调用，无额外授权。只说明接口是否存在，不表示当前包已被授权；未知名字返回 false | [功能查询](api.md#宿主功能查询) |
...
| 尚未公开 | 宿主版本范围、稳定机器错误码 | 已可查询本构建实现了哪些功能。仍只有 `api_version` 精确校验和人读错误文本，没有版本区间或稳定错误码 |
