...
内置 `xuanxue` 将周易、塔罗、吉凶抽签和骰子计算拆为注册、静态数据、抽取、计算和结果格式模块。清单显式声明空能力；普通与只读注册表均按旧开关默认值和显式插件开关提供四个工具。

`ctx.json_integer(pointer)` 用于保留原 JSON 整数表示，使无符号 64 位数量正确收窄，同时保持 `3.0` 等浮点输入原有的默认值行为。固定随机源只在测试入口注入，业务参数不暴露随机控制。完整数据、边界、限额及 330 组原版样本见 [Lua 玄学与骰子](xuanxue.md)。
