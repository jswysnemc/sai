...
图片正文及数据库镜像使用独立缓冲，不经过 Lua JSON 工具结果通道。生成请求、搜索与排序、下载编排、图片元数据、视觉筛选规则和预览策略属于插件；Rust 提供受控网络、字节缓冲、本地文件读取、文件输出、单次视觉请求及终端绘制。
...
`sai.sqlite.query` 读取二进制缓冲中的 SQLite 镜像，`sai.sqlite.apply` 返回新的不可变缓冲。数据库操作只计算内存副本，文件读写仍使用本页已有接口和独立授权，不因数据库计算成功而自动发布。

原缓冲、工作空间和结果使用同一二进制预算；工作线程尚未退出时，即使关闭 Lua 句柄或取消调用也不会提前归还仍占用的额度。镜像必须完整且独立，不读取关联 WAL。结构化请求、固定容量及条件发布示例见 [SQLite 快照接口](sqlite-api.md)。
