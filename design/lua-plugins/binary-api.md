...
新版正式宿主的普通 `write` 与 `write_if` 共用应用状态目录中的固定文件锁，覆盖同一状态目录下不同插件和不同进程。条件比较、完整暂存和发布都在持锁期间完成；暂存结果也继续持锁，清理结束后才释放。固定锁文件不会随输出数量增加，输出接口不能替换锁文件本身。外部编辑器、旧版程序、自定义宿主、其他状态目录或模板进程写入不参与此锁。
