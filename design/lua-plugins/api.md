...
| `tui_status` | TUI 状态变化后计算底栏 | 模型、模式、用量、目录、宽度；见[底栏接口](tui-status-api.md) |
...
除 `tool_call`、`reply_end` 和 `tui_status` 外，事件仅用于观察；返回值不会注入模型消息。监听器失败独立诊断，不替换业务结果，也不阻止其他插件。事件回调没有写入授权，也没有模型和工具调用服务，不能沿用上一工具回调的权限。
...
这项授权只允许生成展示数据，不授予 `sai.notify.send` 主动投递能力；后者使用独立的 `system.notify`，见[主动通知接口](notification-api.md)。
...
`ctx.json_integer` 只存在于工具和事件上下文，查询 JSON 解析后、Lua 转换前的输入快照。`""` 表示根节点，数组索引从 0 开始，`~0`、`~1` 分别转义 `~` 和 `/`；路径必须是 UTF-8 字符串，最多 4096 字节，非法前缀或转义直接报错。路径扫描与返回文本计入共用计算预算，无需外部能力授权。

此查询精确保留有符号及无符号 64 位整数，包括 Lua 无法精确表示为普通数字的 `18446744073709551615`。浮点表示 `3.0`、`3e0`、`-0` 以及超出 JSON 解析器整数范围的数字返回 nil，不把数学上的整数浮点值转换成原整数。修改 Lua 参数表不会改变快照。工具执行或本次事件分发结束、失败或取消后，保存的查询函数失效；弱引用不会让旧上下文保留原始 JSON。该接口不改变 `sai.json.decode` 或普通参数表原有的数字转换方式。
...
快照硬上限为 64 MiB，实际容量继续受共享二进制及执行预算约束；每批最多 64 项变更、512 行写入，查询最多 512 行，JSON 请求和结果最多 1 MiB。固定容量、WAL 输入、结构限制及取消边界见 [SQLite 快照接口](sqlite-api.md)。
...
Lua 计算位于阻塞工作线程，受指令 Hook、堆内存和总时长约束。SQLite 快照复制和虚拟机指令使用同一计算预算，工作及输出预留使用二进制额度；超时和取消不提前释放线程持有的资源。取消释放受管 HTTP、模型、工具、模板进程和通知 Future，并在 Lua 恢复执行时终止回调；通知工作线程收到停止信号，已开始的阻塞文件操作或原生设备初始化不保证强制中断。能力隔离不等于操作系统进程隔离；这里的内存上限针对 Lua 堆，不是整个 Sai 进程。
