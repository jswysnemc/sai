...
Lua 计算位于阻塞工作线程，受指令 Hook、堆内存和总时长约束。取消释放受管 HTTP、模型、工具、模板进程和通知 Future，并在 Lua 恢复执行时终止回调；通知工作线程收到停止信号，已开始的阻塞文件操作或原生设备初始化不保证强制中断。能力隔离不等于操作系统进程隔离；这里的内存上限针对 Lua 堆，不是整个 Sai 进程。
