...
版本 1 提供 HTTP、单次模型请求、显式工具调用、受限文件与环境访问、模板进程、主动通知、JSON、文本和时间能力。私有会话存储、插件跨会话存储、有界归档和工作目录接口见[私有状态与工作目录](private-api.md)。跨插件共享存储、后台定时调度、主 Agent 模型上下文变换和界面组件扩展尚未开放。
