...
`system.notify` 独立控制主动通知投递，通过 `--allow-notify` 与 `--no-notify` 调整。它要求可信写入权限，本地音频另需读取路径授权；与纯展示策略的顶层 `notifications` 无继承关系。完整参数、平台与取消契约见[主动通知接口](notification-api.md)。

`system.schedule` 独立控制本插件命令的持久调度，通过 `--allow-schedule` 与 `--no-schedule` 调整。查询可以只读执行，创建、取消和恢复另需可信写入权限；其他系统授权不会隐式授予调度能力。任务、后台执行及显式恢复契约见[持久调度接口](scheduler-api.md)。
...
环境、文件、模板进程、会话存储、插件持久存储、主动通知与调度操作共用 `limits.system_calls`：默认每次回调 1024 次，上限 4096 次。输入或能力校验失败不执行宿主操作；实际进入宿主后发生错误或超时仍计数。每次工具、命令或事件开始时重置预算，完成、失败和取消后撤销本次系统上下文。

初始化阶段只能读取平台元数据，不能进行环境、文件、进程、通知或调度 I/O。事件可以使用已授权的只读系统能力，但没有模型、工具调用或写入权限，也不能发送通知或修改任务。单次进程或通知时限不能延长回调总截止时间。已经发布的持久任务通过独立工作进程执行，后台命令使用自身的回调预算。

私有会话状态、独立授权的插件跨会话存储、缓存目录、tar.gz 展开及工作目录进程见[私有接口](private-api.md)。普通进程模板默认 `workspace=false`，不能通过私有目录入口改变其工作目录类型。
