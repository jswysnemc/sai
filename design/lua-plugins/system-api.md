...
环境、文件、模板进程、会话存储、插件持久存储与主动通知共用 `limits.system_calls`：默认每次回调 1024 次，上限 4096 次。输入或能力校验失败不执行宿主操作；实际进入宿主后发生错误或超时仍计数。每次工具、命令或事件开始时重置预算，完成、失败和取消后撤销本次系统上下文。

初始化阶段只能读取平台元数据，不能进行环境、文件、进程或通知 I/O。事件可以使用已授权的只读系统能力，但没有模型、工具调用或写入权限，也不能发送通知。单次进程或通知时限不能延长回调总截止时间。
