...
pub fn alarms_file(paths: &SaiPaths) -> PathBuf {
    paths.state_dir.join("alarms.json")
}

pub fn alarm_log_file(paths: &SaiPaths) -> PathBuf {
...
pub fn load(paths: &SaiPaths) -> Result<Vec<AlarmRecord>> {
...
pub fn save(paths: &SaiPaths, records: &[AlarmRecord]) -> Result<()> {
...
pub fn upsert(paths: &SaiPaths, record: AlarmRecord) -> Result<()> {
...
pub fn update_status(paths: &SaiPaths, id: &str, status: AlarmStatus) -> Result<()> {
    let mut records = load(paths)?;
...
pub fn remove(paths: &SaiPaths, id: &str) -> Result<Option<AlarmRecord>> {
    let mut records = load(paths)?;
...
pub fn cleanup_dead(paths: &SaiPaths) -> Result<Vec<AlarmRecord>> {
