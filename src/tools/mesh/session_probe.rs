...
/// 英文版工具说明。
pub(super) const DESCRIPTION_EN: &str = "Inspect live Sai sessions and their open instances, without touching them. scope=all (default) reports live sessions across every workspace; workspace reports this workspace; self reports only the current session. A session is live when a terminal or web page has it open, or a turn is running — including a brand-new session that has not received any prompt yet. Inactive sessions (no live instance or active turn) are omitted, except the current session itself. The sessions array lists ONLY OTHER sessions — your own session is never in it, so any entry in sessions is a valid mesh_send target; your identity is in the top-level self.session_id. Each entry carries the session id and title, whether it is idle (open but not running a turn), the nonexclusive instances, and whether a turn is running. Never use is_workspace_current to decide \"which session am I\": it only names the session this workspace's shared pointer file points at, which is often owned by another terminal or tab. Use it before coordinating across sessions. Read-only: it never starts, switches, or interrupts a session.";

/// 中文版工具说明。
pub(super) const DESCRIPTION_ZH: &str = "查看仍在活动的 Sai 会话及其在线实例,不碰任何会话。scope=all(默认)报所有工作区的活动会话;workspace 报本工作区;self 只报当前会话。终端、网页或网关打开着的会话就算活动,包括还没有发过任何提示词的新会话。没有在线实例或活动轮次的非活动会话不展示,当前会话本身除外。sessions 数组只列出其它会话——你自己的会话绝不会出现在里面,因此 sessions 里任意一条都是合法的 mesh_send 目标;你的身份看顶层 self.session_id。每条记录包含会话 id 与标题、是否空闲(已打开但没在跑一轮)、在线实例以及此刻是否正在跑一轮。绝不要用 is_workspace_current 判断「哪个是我」:它只说明本工作区共享指针文件指向哪条会话,那个会话常常属于另一个终端或标签页。跨会话协作前先查它。只读工具:不会启动、切换或打断会话。";
...
fn session_is_active(session: &LocatedSession, context: &MeshContext) -> bool {
    Path::new(&context.owner_key) == session.state_dir.as_path()
        || !session_instances(&session.state_dir).is_empty()
        || active_run(&session.state_dir).is_some()
}
...
fn describe(session: &LocatedSession, context: &MeshContext) -> Value {
    ...
    let instances = session_instances(state_dir);
    ...
    json!({
        ...
        "online": !instances.is_empty() || running_turn.is_some(),
