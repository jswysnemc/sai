...
pub(crate) fn tool_description(name: &str, fallback: &str) -> String {
    let description = match name {
        ...
        "str_replace" => t(
            "Replace exact text in a file. Match the file content after read_file's line-number and tab prefix; never include that prefix in old_string or new_string.",
            "精确替换文件文本。按 read_file 结果中行号和制表符之后的原文匹配，old_string 与 new_string 都不要包含行号前缀。",
        ),
        ...
        "read_file" => t(
            "Read a file: text with line numbers, images shown to you directly, PDF pages, Jupyter notebooks, or a directory listing.",
            "读取文件：带行号的文本、直接交给你查看的图片、PDF 页面、Jupyter notebook 或目录列表。",
        ),
