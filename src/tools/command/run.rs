...
pub(crate) fn register(
    ...
    registry.register(ToolSpec::new_with_progress(
        "run_command",
        t(
            "Run workspace shell commands for builds, tests, validation, export, inspection, and other command-line programs. Prefer write_file for new files or full rewrites, str_replace for local edits, and edit_file for multi-file Codex patches. Do not use shell redirection, tee, python heredocs, or cat > to edit source files. A program's own -o/--output option may create build artifacts. Wait up to timeout_seconds (default 30, max 120). Use timeout_seconds=0 to start as a background task immediately. If the command is still running when the wait ends, it is promoted to a background task and task_id is returned; manage it with background_command action=list/output/wait/stop/cleanup. Commands that finish within the wait return mode=foreground with full stdout/stderr; any task_id there is audit-only and does not need background_command output or completion handling.",
            "运行构建、测试、校验、导出、检查及其他命令行程序。源码文本修改优先使用 write_file（新建/整文件重写）、str_replace（局部替换）或 edit_file（多文件 Codex 补丁）。不要用 shell 重定向、tee、python heredoc 或 cat > 改源码。程序自身通过 -o/--output 生成构建产物属于允许用途。等待时间由 timeout_seconds 控制（默认 30，最大 120）。timeout_seconds=0 表示立即作为后台任务启动。若等待结束时命令仍在运行，会提升为后台任务并返回 task_id；随后用 background_command 的 list/output/wait/stop/cleanup 管理。若在等待时间内结束，返回 mode=foreground 与完整 stdout/stderr；其中的 task_id 仅供审计，无需再 background_command output，也不会再推送完成回执。",
        ),
        json!({
            "type":"object",
            "properties":{
