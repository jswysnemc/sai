...
pub(crate) fn register(registry: &mut ToolRegistry) {
    registry.register(
        ToolSpec::new(
            "edit_file",
            t(
                "Apply one Codex-style multi-file patch. Preferred form starts with *** Begin Patch and ends with *** End Patch. Common model mistakes are auto-normalized: surrounding markdown fences, *** Begin Patch ***, leading chatter before the envelope, and bare *** Update/Add/Delete File sections. Supports Add File, Update File (with @@ hunks), Delete File, and Move. Every added content line in Add File (including blank lines) must start with +. Update hunk lines must start with space, +, or -. Prefer reading the file first so context lines match. For single-file local edits prefer str_replace; for new files or full rewrites prefer write_file. Do not use shell redirection to edit source.",
                "向工作区应用一个 Codex 风格的多文件补丁。推荐以 *** Begin Patch 开头、*** End Patch 结束。以下常见模型误差会自动归一化：外层 Markdown 代码围栏、*** Begin Patch *** 变体、信封前说明文字、仅有 *** Update/Add/Delete File 段。支持 Add File、Update File（含 @@ hunk）、Delete File 和 Move。Add File 的每一行内容（包括空行）必须以 + 开头。Update hunk 每行以空格、+ 或 - 开头。修改前优先读取文件，确保上下文行匹配。单文件局部修改优先 str_replace；新建或整文件重写优先 write_file。不要用 shell 重定向改源码。",
            ),
