...
fn readable_tool_name_for_locale(name: &str, locale: Locale) -> &str {
    match name {
        ...
        "write_memory" => localized(locale, "Write memory", "写入记忆"),
        "read_memory" => localized(locale, "Read memory", "读取记忆"),
        "list_memory" => localized(locale, "List memory", "列出记忆"),
        "delete_memory" => localized(locale, "Delete memory", "删除记忆"),
