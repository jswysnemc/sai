...
impl StreamRenderer {
    ...
    /// 返回:
    /// - 是否成功
    fn ensure_work_spinner(&mut self, sub_phase: Option<String>) -> Result<()> {
    ...
    /// 返回:
    /// - 是否成功
    fn set_work_status(&mut self, status: WorkStatus, show_spinner: bool) -> Result<()> {
    ...
    /// 返回:
    /// - 是否成功
    fn resume_work_spinner(&mut self) -> Result<()> {
