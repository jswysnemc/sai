...
/// 【终端】【动效测试】不同会话和流式阶段共用进程时钟，切换后不重置动效节拍。
/// 参数：无；返回：无。
#[test]
fn transcript_windows_share_the_animation_clock() {
    let mut first = TranscriptStore::new(100);
    let mut second = TranscriptStore::new(100);
    for store in [&mut first, &mut second] {
        store.set_work_status(crate::render::work_status::WorkStatus::Working);
        assert!(store.advance_live_animation());
    }
    assert_eq!(first.live_animation_started, second.live_animation_started);
    first.push_chunk(&super::test_support::chunk(
...
fn tool_guides_pulse_only_while_running() {
    for (name, args, output) in [
        ...
        let first = store.cells[0].display_lines_framed(80, &options(), 1);
        let later = store.cells[0].display_lines_framed(80, &options(), 8);
        ...
        assert_eq!(
            store.cells[0].display_lines_framed(80, &options(), 1),
            store.cells[0].display_lines_framed(80, &options(), 8),
        );
    }
}
...
fn background_guide_keeps_animating_through_render_cache() {
    ...
    let first = cache.lines_for(0, &store.cells[0], 80, &options(), 1);
    let later = cache.lines_for(0, &store.cells[0], 80, &options(), 8);
    assert_ne!(first[0], later[0], "后台命令引导符不应被定稿缓存冻结");
    assert_eq!(first.len(), later.len());
    assert_eq!(
        cache.count_for(0, &store.cells[0], 80, &options(), 8),
        later.len()
    );
    ...
    cache.invalidate(0);
    let finished = cache.lines_for(0, &store.cells[0], 80, &options(), 1);
    assert_eq!(
        finished,
        cache.lines_for(0, &store.cells[0], 80, &options(), 8)
    );
