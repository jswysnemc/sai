...
impl TranscriptStore {
    ...
    /// 返回:
    /// - 无
    pub(in crate::render::transcript) fn mark_dirty(&mut self, index: usize) {
    ...
    pub(crate) fn finalize_live_tail(&mut self) -> bool {
        ...
        // 思考定稿后标题改过去式；状态行不能再闪 Thinking
        if tail.kind == ChatStreamKind::Reasoning
            && self.work_status == Some(crate::render::work_status::WorkStatus::Thinking)
        {
            self.work_status = Some(crate::render::work_status::WorkStatus::WaitingResponse);
        }
        let index = self.cells.len();
    ...
    /// 返回:
    /// - 是否找到可更新的活动工具
    pub(in crate::render::transcript) fn update_active_tool<F>(
        &mut self,
        name: &str,
        update: F,
    ) -> bool
