...
const EN_TIPS: &[&str] = &[
    ...
    "Ctrl+T toggles the bottom plan between one line and a list",
    "PageUp browses the full transcript without losing your place",
    ...
    "Ctrl+C interrupts a running turn; press twice to exit",
...
const ZH_TIPS: &[&str] = &[
    ...
    "Ctrl+O 打开折叠块全文（←→ 切换）",
    "Ctrl+T 切换底部计划的单行 / 多行",
    "PageUp 浏览完整会话记录（进度不丢失）",
    "Ctrl+V 可粘贴图片或文件到输入框",
    "输入为空时用 ↑ / ↓ 翻阅历史消息",
    "连按两次 Esc 清空当前草稿",
    "模式：yolo · audit · auto · plan",
    "Ctrl+C 中断当前轮次；连按两次退出",
    "智能体工作时按 Tab 将输入加入队列",
