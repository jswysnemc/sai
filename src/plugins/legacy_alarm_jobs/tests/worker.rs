...
/// 【旧闹钟测试】【音频旧许可】旧工作进程不能继承默认授权或增加原音频路径。
/// @returns 无，不播放音频
#[test]
fn legacy_audio_does_not_inherit_grants_or_extend_the_installed_manifest() {
...
async fn legacy_audio_inheritance_never_overrides_explicit_read_scope() {
    ...
    for setting in [
        json!({"enabled":true,"grants":{"system":{"schedule":true,"notify":true}}}),
        json!({"enabled":true,"grants":{"system":{"schedule":true,"notify":true,"read_paths":[]}}}),
    ] {
