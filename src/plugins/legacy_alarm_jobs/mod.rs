...
/// @param paths 可信应用目录
/// @returns 可以通过显式管理入口展示的旧任务
pub(crate) fn list(paths: &SaiPaths) -> Result<Vec<ScheduledTask>> {
    Store::open(paths)?
        .snapshot()?
        .into_iter()
...
/// 【旧闹钟兼容】【单条投影】只接受经过映射的任务标识。
/// @param paths 可信目录；id 为通用任务标识
/// @returns 匹配任务或 None
pub(crate) fn get(paths: &SaiPaths, id: &str) -> Result<Option<ScheduledTask>> {
