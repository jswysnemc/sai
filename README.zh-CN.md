...
- **工作区沙盒** - Audited 模式下,Linux 用 `bubblewrap` 限制文件写入在工作区内,Windows 自动禁用沙盒
...
- **XDG 目录** - Linux 遵循 XDG 规范,Windows 映射到 `%APPDATA%` / `%LOCALAPPDATA%` 标准位置
...
每次推送到 `main` 分支会触发 GitHub Actions 构建 Linux 与 Windows 二进制,前往 [Actions](https://github.com/SHORiN-KiWATA/Sai/actions) 页面下载对应平台的 artifact。
...
编辑配置文件(Linux `~/.config/sai/config.jsonc`,Windows `%APPDATA%\sai\config.jsonc`):
...
Sai 遵循 XDG 规范(Linux)与系统标准目录(Windows),所有路径可用 `sai paths` 查看。
...
Linux `~/.local/share/sai` / Windows 数据目录
...
- 缓存:Linux `~/.cache/sai`
- 图片产物:Linux `~/Pictures/sai` / Windows `Pictures\sai`
...
不能。审计沙盒依赖 Linux `bubblewrap`,Windows 上 Audited 模式仍可用审计日志与逐次确认,但文件写入沙盒不启用。
