...
- **Lua 插件** - 安装本地 Lua 5.4 包，扩展工具、用户命令和生命周期回调；配置独立保存，HTTP 来源和只读 POST 查询端点需明确授权。在线手册、DeepSeek 状态、Arch Linux 查询、Fcitx Wiki、ProtonDB 及六个网页搜索供应商已迁为内置 Lua 实现。参见[开发指南](design/lua-plugins/getting-started.md)、[接口说明](design/lua-plugins/api.md)和[迁移进度](design/lua-plugins/migration.md)。
