...
- **Lua 插件** - 八个 Lua 5.4 内置包提供 13 个工具，并支持用户命令、生命周期回调与独立配置；HTTP、模型和工具能力分别授权。在线手册、DeepSeek 状态、Arch Linux 查询、Fcitx Wiki、ProtonDB、六个网页搜索供应商及完整 Linux 游戏调查已迁为 Lua 实现。插件通过受限宿主服务使用当前 Agent 的模型并组合已授权工具。参见[开发指南](design/lua-plugins/getting-started.md)、[接口说明](design/lua-plugins/api.md)和[迁移进度](design/lua-plugins/migration.md)。
