...
- **Lua 插件** - 十四个 Lua 5.4 内置包提供 21 个工具，支持用户命令、生命周期回调、独立配置及逐项能力授权。在线查询、游戏与输入法调查、系统证据采集及完整 AUR 审查与安装流程已迁入 Lua。插件使用当前 Agent 的模型与已授权工具，通过通用宿主接口访问有界文件、进程、私有会话状态和工作目录。参见[开发指南](design/lua-plugins/getting-started.md)、[接口说明](design/lua-plugins/api.md)和[迁移进度](design/lua-plugins/migration.md)。
