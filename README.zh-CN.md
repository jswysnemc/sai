...
- **Lua 插件** - 安装本地 Lua 5.4 包，扩展工具、用户命令和生命周期回调；配置独立保存，HTTP 来源需明确授权。在线手册、DeepSeek 状态、Arch Linux 查询、Fcitx Wiki 和 ProtonDB 已迁为内置 Lua 包。参见[开发指南](design/lua-plugins/getting-started.md)、[接口说明](design/lua-plugins/api.md)和[迁移进度](design/lua-plugins/migration.md)。
