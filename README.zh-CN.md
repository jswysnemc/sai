...
- **Lua 插件** - 九个 Lua 5.4 内置包提供 14 个工具，并支持用户命令、生命周期回调与独立配置；HTTP、模型和工具能力分别授权。在线手册、DeepSeek 状态、Arch Linux 查询、Fcitx Wiki、ProtonDB、六个网页搜索供应商及完整 Linux 游戏、输入法调查已迁为 Lua 实现。插件通过受限宿主服务使用当前 Agent 的模型，组合已授权工具，并为每次工具调用设置超时。参见[开发指南](design/lua-plugins/getting-started.md)、[接口说明](design/lua-plugins/api.md)和[迁移进度](design/lua-plugins/migration.md)。
...
后台命令卡保留 `$` 命令行并自动更新近期日志，长输出沿用 `Ctrl+O` 折叠查看。后台任务连续 90 秒无新输出，或运行达到 10 分钟时，会提醒主代理检查进展；确认检查后至少间隔 5 分钟再次提醒。单次等待最长 60 秒，到期返回状态与近期日志，不会因此停止命令。待办面板使用 `Ctrl+T` 展开或折叠，子任务面板使用 `↓` 展开并选择任务。
