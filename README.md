...
- **Lua plugins** - Install local Lua 5.4 packages with tools, user commands, and lifecycle callbacks. Plugins use separate settings and explicit HTTP grants; online manuals, DeepSeek status, Arch Linux queries, Fcitx Wiki, ProtonDB, and all six web search providers use bundled Lua implementations. Read-only POST queries require explicit endpoint grants. See the [plugin guide](design/lua-plugins/getting-started.md), [API](design/lua-plugins/api.md), and [migration status](design/lua-plugins/migration.md) (Chinese).
