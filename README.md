...
- **Lua plugins** - Seventeen local Lua 5.4 packages provide 23 tools, with user commands, lifecycle callbacks, separate settings, and explicit HTTP, model, tool, file, environment, process, private storage, workspace, binary download, file output, and image display grants. Online manuals, DeepSeek status, Arch Linux queries, Fcitx Wiki, ProtonDB, six web search providers, weather, exchange rates, Moegirlpedia, complete Linux game and input method investigations, local diagnostic evidence, AUR review/install workflows, reply notification policy, and image generation and display use bundled Lua implementations. Plugins compose authorized tools and use the current Agent's model through bounded host services. Image generation saves files before invoking the independently configured display plugin for optional preview. See the [plugin guide](design/lua-plugins/getting-started.md), [API](design/lua-plugins/api.md), and [migration status](design/lua-plugins/migration.md) (Chinese).
