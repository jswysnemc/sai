...
- **Lua plugins** - Eight local Lua 5.4 packages provide 13 tools, with user commands, lifecycle callbacks, separate settings, and explicit HTTP, model, and tool grants. Online manuals, DeepSeek status, Arch Linux queries, Fcitx Wiki, ProtonDB, six web search providers, and the complete Linux game investigation use bundled Lua implementations. Plugins compose authorized tools and use the current Agent's model through bounded host services. See the [plugin guide](design/lua-plugins/getting-started.md), [API](design/lua-plugins/api.md), and [migration status](design/lua-plugins/migration.md) (Chinese).
