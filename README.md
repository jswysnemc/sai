...
- **Lua plugins** - Thirteen local Lua 5.4 packages provide 19 tools, with user commands, lifecycle callbacks, separate settings, and explicit HTTP, model, tool, file, environment, and process grants. Online manuals, DeepSeek status, Arch Linux queries, Fcitx Wiki, ProtonDB, six web search providers, weather, exchange rates, Moegirlpedia, complete Linux game and input method investigations, and local diagnostic evidence use bundled Lua implementations. Plugins compose authorized tools and use the current Agent's model through bounded host services. Read-only diagnostics keep application launch and version execution behind a separate writing tool. See the [plugin guide](design/lua-plugins/getting-started.md), [API](design/lua-plugins/api.md), and [migration status](design/lua-plugins/migration.md) (Chinese).
