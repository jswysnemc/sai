...
export function ChangeContextMenu(props: ChangeContextMenuProps) {
  ...
  return (
    <div ref={rootRef} className="git-change-context-menu" role="menu" style={{ left: props.x, top: props.y }}>
      ...
      {discardPaths.length > 0 && <Button variant="ghost-danger" disabled={props.busy} onClick={() => void discard()}>{props.entries.some((entry) => entry.untracked) ? <Trash2 size={12} /> : <RotateCcw size={12} />}{t("Discard Selected", "丢弃所选改动")}</Button>}
      <span>{t("Path", "路径")}</span>
