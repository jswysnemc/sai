...
export function ContextBannerNavigation({ sections, onSection, onTop, onBottom, onClose }: {
  ...
  return <nav className="context-banner-navigation" aria-label={t("Context navigation", "上下文导航")}>
    ...
    <div className="context-banner-navigation-actions">
      <Button variant="ghost" size="icon" onClick={onTop} aria-label={t("Go to context top", "前往上下文顶部")} title={t("Top", "顶部")}><ArrowUpToLine size={14} /></Button>
      <Button variant="ghost" size="icon" onClick={onBottom} aria-label={t("Go to context bottom", "前往上下文底部")} title={t("Bottom", "底部")}><ArrowDownToLine size={14} /></Button>
      <Button variant="ghost" size="icon" onClick={onClose} aria-label={t("Close context", "关闭上下文")} title={t("Close", "关闭")}><X size={14} /></Button>
    </div>
  </nav>;
}
