...
export function SidebarTaskBoard(props: SidebarTaskBoardProps) {
  ...
  useEffect(() => {
    ...
    setRecents(rememberRecent("sessions", props.selectedSessionId));
  ...
  const pinnedIds = new Set(index.data?.pinned ?? []);
  const sessions = sortSessions(flattenSessions(props.workspaces), "updated");
  const pinned = sessions.filter((item) => pinnedIds.has(item.session.id));
  const unpinnedSessions = sessions.filter((item) => !pinnedIds.has(item.session.id));
  const sessionSplit = splitByRecent(unpinnedSessions, (item) => item.session.id, recents.sessions, recents.sessionBaseline);
  ...
  const sessionOrder = unpinnedSessions.map((item) => item.session.id).join("|");
  ...
  const rowActions = {
    ...
    onOpenSession: (workspaceId: string, sessionId: string, workspaceActive: boolean, sessionActive: boolean) => {
      setRecents(rememberRecent("sessions", sessionId));
      ...
      {props.browse === "workspaces" && (props.loading ? <SkeletonList items={3} label={t("Loading sessions", "读取会话")} /> : props.workspaces.length > 0 ? (
        <>
          <WorkspaceListView
            ...
            onOpenWorkspace={(workspaceId, active) => {
              setRecents(rememberRecent("workspaces", workspaceId));
              props.onOpenWorkspace(workspaceId, active);
            }}
