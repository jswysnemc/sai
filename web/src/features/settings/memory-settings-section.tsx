  ...
  return (
    <section className="settings-section-card">
      <header className="settings-section-head">
        <h2><Brain size={16} /> 记忆管理</h2>
        <p>查看、搜索和管理长期事实与往事；不会改动当前会话历史。</p>
      </header>

      <div className="memory-stats">
        <div>事实：{String(stats.data?.facts ?? "—")}</div>
        <div>往事：{String(stats.data?.episodes ?? "—")}</div>
        <div>裁剪上下文：{String(stats.data?.evicted_turns ?? "—")}</div>
        <button type="button" className="settings-secondary" onClick={() => reset.mutate()} disabled={reset.isPending}>
          {reset.isPending ? "清空中…" : "清空记忆"}
        </button>
      </div>

      <div className="memory-compose">
        <textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="写入一条新事实…" rows={3} />
        <button type="button" onClick={() => remember.mutate()} disabled={!draft.trim() || remember.isPending}>
          <Plus size={14} /> {remember.isPending ? "保存中" : "记住"}
        </button>
      </div>
      ...
      <MemoryList title="事实" items={facts} onRemove={(id) => remove.mutate({ kind: "fact", id })} />
      <MemoryList title="往事" items={episodes} onRemove={(id) => remove.mutate({ kind: "episode", id })} />
...
function MemoryList({
  ...
  return (
    <div className="memory-list-block">
      ...
      {items.length === 0 && <div className="settings-muted">暂无</div>}
      <div className="memory-list">
        {items.map((item) => (
          <article key={`${item.kind}-${item.id}`} className="memory-item">
            <p>{item.content}</p>
            <footer>
              <small>{item.source} · {item.status} · {new Date(item.updated_at).toLocaleString()}</small>
              <button type="button" onClick={() => onRemove(item.id)} aria-label="删除记忆">
                <Trash2 size={13} />
              </button>
            </footer>
          </article>
