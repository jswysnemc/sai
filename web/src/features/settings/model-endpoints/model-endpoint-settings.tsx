  ...
  return (
    <div className="settings-objects-layout">
      ...
      <div className="settings-editor min-w-0">
        ...
        {selected ? <SettingsGroup title={t("Connection", "连接配置")}>
          <div className="grid min-w-0 grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="min-w-0 md:col-span-2">
              <TextFieldRow label={t("Request endpoint", "请求地址")} value={selected.endpoint} placeholder="https://example.com:8443/api/model" onChange={(endpoint) => patch({ endpoint })} hint={t("Complete HTTP(S) request URL, including any port and path.", "完整 HTTP(S) 请求地址，包含所需端口和路径。")} />
            </div>
            <div className="settings-field min-w-0 md:col-span-2">
              <label htmlFor={`model-endpoint-key-${selected.id}`}>API Key</label>
              <PasswordField key={selected.id} id={`model-endpoint-key-${selected.id}`} ariaLabel={t("API key", "API Key")} value={hidden ? "" : selected.api_key} savedValueHint={hidden ? t("Saved", "已保存") : undefined} placeholder={hidden ? t("Enter a new key to replace it", "输入新 Key 以替换") : "$env:MODEL_API_KEY"} onClearSavedValue={hidden ? () => patch({ api_key: "" }) : undefined} onChange={(api_key) => patch({ api_key })} />
              <small>{t("Stored independently from LLM provider keys. Supports $env:VARIABLE references.", "与 LLM 供应商的 Key 独立保存，支持 $env:变量名 引用。")}</small>
            </div>
          </div>
        </SettingsGroup> : <div className="settings-empty"><p>{t("No model connections configured.", "尚未配置模型接入。")}</p><Button className="settings-secondary" onClick={add}>{t("Add model connection", "新增模型接入")}</Button></div>}
      </div>
    </div>
  );
}
