...
export function ModelIcon({model,size=16}:ModelIconProps){
  ...
  return <span aria-label={t(`Model ${model}`, `模型 ${model}`)} style={{display:"inline-grid",width:size,height:size,placeItems:"center",borderRadius:4,background:"var(--graphite-soft,#394244)",color:"#eef2f0",fontSize:Math.max(8,size*.48),fontWeight:600}}>{model.slice(0,2)}</span>;
}
